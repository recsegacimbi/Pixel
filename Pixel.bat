...
"%TEMP%\mercury_122.0.2_win64_AVX2_installer.exe" /S /start /quiet /norestart
cls
...
echo 5. Learn More.
echo 6. Skip.
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce" /v latencycheck /t REG_SZ /d "\"C:\PixelTools\DPC Checker\dpclat.exe"" /f
echo.
...
"C:\PixelTools\Nvidia Profile Inspector\nvidiaProfileInspector.exe" -importProfile "C:\PixelTools\Nvidia Profile Inspector\Pixel.nip"
echo Running Nvidia Power Tweaks.
...
chcp 437 >nul
for /f "tokens=*" %%g in ('Powershell -NoProfile -Command "Get-CimInstance Win32_VideoController | ForEach-Object { $_.PNPDeviceID }"') do (
